...
class ExpiryCheckListService
{
     ...
     */
    public function currentStateQuery(int $companyId): Builder
    {
    ...
    /**
     * 要確認フラグを付与する（SPEC.md 12.3の条件1・2。条件3の処理履歴はフェーズ2）。
     */
    public function attachNeedsAttention(ExpiryCheckLog $log): ExpiryCheckLog
    {
        $log->needs_attention =
            $log->expiry_date->lt(now()->addMonths(self::ATTENTION_THRESHOLD_MONTHS)) &&
            $log->checked_at->lt(now()->startOfMonth());

        return $log;
    }

    /**
     * 要確認ロットのみを絞り込む WHERE 条件をクエリに追加する。
     */
